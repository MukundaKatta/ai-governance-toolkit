"""FastAPI application for ai-governance-toolkit."""
...
app = FastAPI(title="ai-governance-toolkit", version="0.1.0", description="ai-governance-toolkit API")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "ai-governance-toolkit", "version": "0.1.0"}
...
async def process(req: ProcessRequest):
    ...
    try:
        ...
        resp = ProcessResponse(success=True, result=result, processing_time_ms=round(elapsed, 2),
                              metadata={"service": "ai-governance-toolkit", "input_length": len(req.input)})
...
@app.get("/status")
def status():
    return {"service": "ai-governance-toolkit", "version": "0.1.0", "ready": True,
